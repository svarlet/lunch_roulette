defmodule Web.SubmitRestaurantControllerTest do
  ...
  describe "the submission of an invalid restaurant" do
    ...
    test "report error with a flash message", %{conn: conn, name: name} do
      assert %{"error" => "Error: irrelevant_category of #{name} failed because of irrelevant_reason."} == get_flash(conn)
    end
  end
end
